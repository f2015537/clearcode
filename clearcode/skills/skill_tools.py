...
from clearcode.skills.registry import SkillRegistry, SkillNotFoundError
...
def get_registry() -> SkillRegistry:
   ...
   skills_dir resolution order:
     1. Read "skills.skills_dir" from config.yaml  →  e.g. ".clearcode/skills"
     2. Resolve relative to Path.cwd()             →  the target project root
        (clearcode is invoked from the project being analysed, so cwd()
         IS the target project, not the clearcode package itself)


   Example:
     User runs clearcode from /home/user/my_project/
     config.yaml says skills_dir: .clearcode/skills
     → skills are loaded from /home/user/my_project/.clearcode/skills/
   """
   global _registry
   if _registry is None:
       skills_dir = Path.cwd() / config.get("skills", {}).get("skills_dir", ".clearcode/skills")
