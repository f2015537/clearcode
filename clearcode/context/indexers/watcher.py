...
from clearcode.context.indexers.code_parser import ALL_EXTENSIONS
...
class _CodebaseEventHandler(FileSystemEventHandler):
   ...
   def _run(self, action: str, filepath: str) -> None:
       ...
       time and to pick up the correct backend from config at call time.
       """
       self._timers.pop(filepath, None)
       from clearcode.context.indexers.factory import get_single_file_indexer, get_file_remover
       if action == "delete":
