...
class SQLiteTaskStore:
   ...
   def __init__(self, db_path: str = ".clearcode/tasks/tasks.db") -> None:
   ...
   def finish_project(self, project_id: str) -> None:
       """Mark a project completed so the next /plan creates a fresh one."""
       with self._conn() as conn:
           conn.execute(
               "UPDATE projects SET status='completed' WHERE id=?",
               (project_id,),
           )
       logger.info(f"Project {project_id} marked completed")
