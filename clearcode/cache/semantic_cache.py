...
class SemanticCache:
   ...
   async def invalidate_domain(self, domain: str) -> None:
       ...
       Uses the domain Tag index to find matching keys in one query, then
       deletes them in a single bulk call — O(matches) instead of O(all keys).
       """
       from redisvl.query import FilterQuery
       fq = FilterQuery(
           filter_expression=Tag("domain") == domain,
           return_fields=[],
           num_results=10_000,
       )
