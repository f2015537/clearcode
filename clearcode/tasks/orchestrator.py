...
from clearcode.tasks.task_store import SQLiteTaskStore
from clearcode.tasks.executor import run_subtask_agent
from clearcode.tasks.planner import create_plan
from clearcode.tasks.approval import present_plan_for_approval
from clearcode.tasks.recovery import RecoveryManager
from clearcode.context.indexers.factory import get_indexer
...
async def handle_plan_command(goal: str) -> None:
   ...
   """
   db_path = config.get("tasks", {}).get("db_path", ".clearcode/tasks/tasks.db")
   ...
   if project_id:
       ...
       while approved_plan is None:
           ...
           approved_plan = await present_plan_for_approval(raw_plan)
   ...
   store.finish_project(project_id)
   ...
   try:
       get_indexer()(str(Path.cwd()), force=True)
