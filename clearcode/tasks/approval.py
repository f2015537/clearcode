...
from clearcode.tasks.planner import ExecutionPlan, PlannedTask
...
async def present_plan_for_approval(plan: ExecutionPlan) -> ExecutionPlan | None:
   """
   ...
   while True:
       ...
       if choice == "A":
           ...
           task_id = (await asyncio.to_thread(input, "Enter task ID to modify: ")).strip()
           task = next((t for t in plan.tasks if t.id == task_id), None)
           if not task:
               console.print(f"[red]Task '{task_id}' not found.[/red]")
               continue
           console.print(f"\nCurrent description:\n{task.description}\n")
           new_desc = (await asyncio.to_thread(input, "New description: ")).strip()
           if new_desc:
