...
def _parse_with_treesitter(
    source: str, filepath: str, language_name: str
) -> list[ParsedChunk]:
    ...
    if not chunks:
        logger.warning(
            f"No AST blocks found in {filepath}, falling back to sliding window"
        )
        return _sliding_window(lines, filepath)
...
def _walk(node, source_bytes: bytes, filepath: str, chunks: list, depth: int):
    ...
    if node.type in BLOCK_NODE_TYPES:
        ...
        logger.debug(
            f"  Found {chunk_type} '{name}' (lines {node.start_point[0]+1}-{node.end_point[0]+1})"
        )
