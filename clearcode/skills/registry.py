...
class SkillRegistry:
    ...
    After calling load(), the internal _skills dict looks like:
        {
            "python_debug": {
                ...
                "skill_dir": Path(".clearcode/skills/python_debug")
            },
            "write_tests": {
                "meta":      {"name": "write_tests", "description": "...", ...},
                "body":      "Generate pytest unit tests...",
                "skill_dir": Path(".clearcode/skills/write_tests")
            }
        }
    ...
    def load(self) -> None:
        ...
        if not self._skills_dir.exists():
            logger.warning(
                f"skills_dir not found: {self._skills_dir}. No skills loaded."
            )
            return

        candidates = [d for d in self._skills_dir.iterdir() if d.is_dir()]
        logger.debug(f"Found {len(candidates)} candidate skill folders")
        ...
        for skill_dir in candidates:
            ...
            try:
                ...
                logger.info(
                    f"Loaded skill '{name}' from {skill_dir.name} "
                    f"({len(body)} chars, {len(_list_support_files(skill_dir))} support files)"
                )
            except Exception as e:
                logger.error(f"Failed to load skill from {skill_dir.name}: {e}")

        logger.info(f"Skills loaded: {len(self._skills)} total — {list(self._skills.keys())}")
    ...
    def load_skill(self, name: str) -> str:
        ...
        logger.debug(
            f"Skill '{name}': {len(body)} chars body, {len(support_files)} support files"
        )
